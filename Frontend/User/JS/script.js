...
function ViewCategories(categories) {
  ...
  categories.forEach((categorie) => {
    const decryptedName = decryptData(categorie.name); // Decrypt category name
    categorieshtml += `            
    <button class="btn btn-warning  btn-outline-dark  border-warning  mx-lg-5" onClick="searchBycategorie('${decryptedName}')">
    ${decryptedName}</button>
      `;
  });
